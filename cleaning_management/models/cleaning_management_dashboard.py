...
class CleaningManagementDashBoard(models.Model):
    ...
    def get_the_booking_year(self):
        ...
        total_booking_stage_year = cleaning.search([]).filtered(
            lambda l: l.booking_date and l.booking_date.year == fields.date.today().year).mapped(
            'state')
        order_of_stages = ['draft', 'booked', 'cleaned', 'cancelled']
        year_stage = [stage for stage in set(total_booking_stage_year) if stage in order_of_stages]
        # Sort the stages based on the predefined order
        sorted_stages = sorted(year_stage,
                               key=lambda x: order_of_stages.index(x))
        stage_map = {'draft': 'Draft', 'booked': 'WO Running', 'cleaned': 'Team Done', 'cancelled': 'Cancelled'}
        year_booking_stages = [stage_map.get(stage, stage.capitalize()) for stage in sorted_stages]
        total_booking_stage_draft_year = cleaning.search([
            ('state', '=', 'draft')]).filtered(
            lambda l: l.booking_date and l.booking_date.year == fields.date.today().year)
    ...
    def get_the_booking_month(self):
        ...
        total_booking_stage_month = cleaning.search([]).filtered(
            lambda l: l.booking_date and l.booking_date.month == fields.date.today().month).mapped(
            'state')
        order_of_stages = ['draft', 'booked', 'cleaned', 'cancelled']
        month_stage = [stage for stage in set(total_booking_stage_month) if stage in order_of_stages]
        # Sort the stages based on the predefined order
        sorted_stages = sorted(month_stage,
                               key=lambda x: order_of_stages.index(x))
        stage_map = {'draft': 'Draft', 'booked': 'WO Running', 'cleaned': 'Team Done', 'cancelled': 'Cancelled'}
        monthly_booking_stages = [stage_map.get(stage, stage.capitalize()) for stage in sorted_stages]
        total_booking_stage_draft_month = cleaning.search(
            [('state', '=', 'draft')]).filtered(
            lambda l: l.booking_date and l.booking_date.month == fields.date.today().month)
    ...
    def get_the_booking_week(self):
        ...
        total_booking_stage_week = cleaning.search([]).filtered(
            lambda l: l.booking_date and l.booking_date.isocalendar()[1] ==
                      fields.date.today().isocalendar()[1]).mapped('state')
        order_of_stages = ['draft', 'booked', 'cleaned', 'cancelled']
        week_stage = [stage for stage in set(total_booking_stage_week) if stage in order_of_stages]
        # Sort the stages based on the predefined order
        sorted_stages = sorted(week_stage,
                               key=lambda x: order_of_stages.index(x))
        stage_map = {'draft': 'Draft', 'booked': 'WO Running', 'cleaned': 'Team Done', 'cancelled': 'Cancelled'}
        weekly_booking_stages = [stage_map.get(stage, stage.capitalize()) for stage in sorted_stages]
    ...
    def get_the_booking_quarter(self):
        ...
        total_booking_stage_quarter = cleaning.search([]).filtered(
            lambda l: l.booking_date and start_date <= l.booking_date <= end_date).mapped('state')
        order_of_stages = ['draft', 'booked', 'cleaned', 'cancelled']
        quarter_stage = [stage for stage in set(total_booking_stage_quarter) if stage in order_of_stages]
        # Sort the stages based on the predefined order
        sorted_stages = sorted(quarter_stage,
                               key=lambda x: order_of_stages.index(x))
        stage_map = {'draft': 'Draft', 'booked': 'WO Running', 'cleaned': 'Team Done', 'cancelled': 'Cancelled'}
        quarterly_booking_stages = [stage_map.get(stage, stage.capitalize()) for stage in sorted_stages]
        total_booking_stage_draft_quarter = cleaning.search([
            ('state', '=', 'draft')]).filtered(
            lambda l: l.booking_date and start_date <= l.booking_date <= end_date)
    ...
    def quality_year(self):
        ...
        quality_year = quality.search([]).filtered(
            lambda
                l: l.inspection_date_and_time and l.inspection_date_and_time.year == fields.date.today().year).mapped(
            'state')
        order_of_stages = ['cleaned', 'dirty']
        year_stage = [stage for stage in set(quality_year) if stage in order_of_stages]
        # Sort the stages based on the predefined order
        sorted_stages = sorted(year_stage,
                               key=lambda x: order_of_stages.index(x))
        stage_map = {'cleaned': 'Done', 'dirty': 'Undone', 'draft': 'Draft'}
        yearly_quality_stages = [stage_map.get(stage, stage.capitalize()) for stage in sorted_stages]
        cleaned_quality_year = quality.search([
            ('state', '=', 'cleaned')]).filtered(
            lambda
                l: l.inspection_date_and_time and l.inspection_date_and_time.year == fields.date.today().year)
    ...
    def quality_month(self):
        ...
        quality_month = quality.search([]).filtered(
            lambda
                l: l.inspection_date_and_time and l.inspection_date_and_time.month == fields.date.today().month).mapped(
            'state')
        order_of_stages = ['cleaned', 'dirty']
        month_stage = [stage for stage in set(quality_month) if stage in order_of_stages]
        # Sort the stages based on the predefined order
        sorted_stages = sorted(month_stage,
                               key=lambda x: order_of_stages.index(x))
        stage_map = {'cleaned': 'Done', 'dirty': 'Undone', 'draft': 'Draft'}
        monthly_quality_stages = [stage_map.get(stage, stage.capitalize()) for stage in sorted_stages]
        cleaned_quality_month = quality.search([
            ('state', '=', 'cleaned')]).filtered(
            lambda
                l: l.inspection_date_and_time and l.inspection_date_and_time.month == fields.date.today().month)
    ...
    def quality_week(self):
        ...
        quality_week = quality.search([]).filtered(
            lambda l: l.inspection_date_and_time and l.inspection_date_and_time.isocalendar()[1] ==
                      fields.date.today().isocalendar()[1]).mapped('state')
        order_of_stages = ['cleaned', 'dirty']
        week_stage = [stage for stage in set(quality_week) if stage in order_of_stages]
        # Sort the stages based on the predefined order
        sorted_stages = sorted(week_stage,
                               key=lambda x: order_of_stages.index(x))
        stage_map = {'cleaned': 'Done', 'dirty': 'Undone', 'draft': 'Draft'}
        capitalized_stages = [stage_map.get(stage, stage.capitalize()) for stage in sorted_stages]
    ...
    def quality_quarter(self):
        ...
        quality_quarter = quality.search([]).filtered(
            lambda
                l: l.inspection_date_and_time and start_date <= l.inspection_date_and_time <= end_date).mapped(
            'state')
        order_of_stages = ['cleaned', 'dirty']
        quarter_stage = [stage for stage in set(quality_quarter) if stage in order_of_stages]
        # Sort the stages based on the predefined order
        sorted_stages = sorted(quarter_stage,
                               key=lambda x: order_of_stages.index(x))
        stage_map = {'cleaned': 'Done', 'dirty': 'Undone', 'draft': 'Draft'}
        ...
        cleaned_quality_quarter = quality.search([
            ('state', '=', 'cleaned')]).filtered(
            lambda l: l.inspection_date_and_time and start_date <= l.inspection_date_and_time <= end_date)
